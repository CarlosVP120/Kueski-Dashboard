...
const getColumnNames = (table) => {
  return new Promise((resolve, reject) => {
    let sqlQuery = `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '${table}';`;
...
const updateOneUser = (userId, changes) => {
  if (changes.oposition_rules) {
    // UPDATE the user's oposition rules JSON object
    return new Promise((resolve, reject) => {
      changes.oposition_rules = JSON.stringify(changes.oposition_rules);
      let sqlQuery = "UPDATE users SET oposition_rules = ? WHERE user_id = ?;";
      db.query(sqlQuery, [changes.oposition_rules, userId], (error, result) => {
        if (error) {
          console.log(error);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const key in changes) {
      ...
      if (identificationsColumns.includes(key))
        updatedTables.add("identifications");
      sqlQuery += `${key} = ?, `;
